...
void polish(struct stack *pt, FILE *fp, char *filename) // решение выражения в обратной польской записи
{
    ...
    while (!feof(fp))          // цикл для чтения файла
    {
        ...
        if (ch == ',') // проверка ch на равенство запятой
        {
            ch = '.'; // замена ch на точку
        }
        ...
        else
        {
            if (ch == ' ') // проверка символа на пробел
            {
                ...
                {
                    double numb = strtod(str, &endptr); // генерация числа из строки
...
{
    fp = fopen(filename, "r"); // открытие файла
    int curr = 2;              // переменная для текущего количества чисел под ряд
    int maximum = 2;           // переменная для максимального количества чисел под ряд
